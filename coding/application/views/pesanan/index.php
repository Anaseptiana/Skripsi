                ...
                <div class="card-body">
                    <table class="table table-hover table-bordered">
                        ...
                        <tbody>
                            ...
                            ?>
                            <?php $i = 1; ?>
                            <?php foreach ($pesanans as $td) : ?>
                                <tr>
                                    <td><?= $i; ?></td>
                                    <td><?= $td['tanggal_pesan']; ?></td>
                                    <td><?= $td['no_pesanan']; ?></td>
                                    <td><?= $td['nama_customer']; ?></td>
                                    <td>Rp. <?= numberFormat($td['harga_mobil']); ?></td>
                                    <td>Rp. <?= numberFormat($td['harga_supir']); ?></td>
                                    <td><?= $td['jumlah_hari']; ?></td>
                                    <td>Rp. <?= numberFormat($td['total_harga']); ?></td>
                                    <td><?= statusPesanan($td['status']); ?></td>
                                    <td class="text-center">
                                        <!-- <a href="<?= base_url('administrator/pesanan/detail/' . $td['id_pesanan']); ?>" class="badge badge-info">Detail</a> -->
                                        <a href="<?= base_url('administrator/pesanan/edit/' . $td['id_pesanan']); ?>" class="badge badge-success">Detail</a>
                                    </td>
                                </tr>
                                <?php $i++; ?>
                            <?php endforeach; ?>
